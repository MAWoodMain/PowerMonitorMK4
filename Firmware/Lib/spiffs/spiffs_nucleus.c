...
s32_t spiffs_phys_rd(
    ...
    u32_t len,
    u8_t *dst) {
  return SPIFFS_HAL_READ(fs, addr, len, dst, fs->cfg.cardFs, fs->cfg.spi, fs->cfg.channel);
}
...
s32_t spiffs_phys_wr(
    ...
    u32_t len,
    u8_t *src) {
  return SPIFFS_HAL_WRITE(fs, addr, len, src, fs->cfg.cardFs, fs->cfg.spi, fs->cfg.channel);
}
...
s32_t spiffs_erase_block(
  ...
  while (size > 0) {
    ...
    SPIFFS_HAL_ERASE(fs, addr, SPIFFS_CFG_PHYS_ERASE_SZ(fs), fs->cfg.cardFs, fs->cfg.spi, fs->cfg.channel);
